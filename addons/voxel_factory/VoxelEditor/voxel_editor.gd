...
func _ready():
	open_dialog = get_node("OpenDialog")
...
func _physics_process(delta):
	...
	# Raycasts to detect which voxel we are aimint at.
	var from = camera.project_ray_origin(mouse_position) # from camera.
	var to = from + camera.project_ray_normal(mouse_position) * 60 # ray length 30
	...
	# SPACE SHORTCUT
	if Input.is_key_pressed(KEY_SPACE) && current_tool != TOOLS.move:
	...
	elif !Input.is_key_pressed(KEY_SPACE) && current_tool != last_tool:
